...
function CarMade() {
  ...
  const DeleteCarMade = () => {
    axios
      ...
      .then((res) => {
        ...
        axios
          .get(
            `/car-mades?page=${page}&ordered_by=${sortModel[0].field}&sort_type=${sortModel[0].sort}`,
            {
              headers: {
                Authorization: `Bearer ${user.token}`,
              },
  ...
  useEffect(() => {
    ...
    if (!userIsSearching) {
      ...
      axios
        .post(
          `/car-mades/search/name?page=${page}&ordered_by=${sortModel[0].field}&sort_type=${sortModel[0].sort}`,
          {
