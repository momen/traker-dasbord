...
import CarTypesForm from "./CarTypesForm";
...
function CarTypes() {
  ...
  const [openPopupTitle, setOpenPopupTitle] = useState("New Car Type"); // Customize
  ...
  const [pageHeader, setPageHeader] = useState("Car Types");
            ...
            {userPermissions.includes("car_type_update") ? (
              <Button
                ...
                onClick={() => {
                  ...
                  // setOpenPopup(true);
                  // setOpenPopupTitle("Edit Car Type");
                  setViewMode("edit");
                  setPageHeader("Edit Car Type");
                }}
              >
                Edit
              </Button>
  ...
  const DeleteItem = () => {
    axios
      .delete(`/car-types/${itemToDelete}`)
      .then((res) => {
        setOpenDeleteDialog(false);
        setLoading(true);
        axios
          .get(
            `/car-types?page=${page}&per_page=${pageSize}&ordered_by=${sortModel[0].field}&sort_type=${sortModel[0].sort}`
          )
          .then((res) => {
  ...
  useEffect(() => {
    ...
    if (!userIsSearching) {
      ...
      axios
        .post(
          `/car-types/search/name?page=${page}&per_page=${pageSize}&ordered_by=${sortModel[0].field}&sort_type=${sortModel[0].sort}`,
          {
  ...
  return (
    <React.Fragment>
          ...
          <Paper>
            <div style={{ width: "100%" }}>
              <DataGrid
                ...
                autoHeight={true}
                onRowClick={
                  userPermissions.includes("car_type_show")
                    ? ({ row }) => history.push(`/product/car-types/${row.id}`)
                    : null
                }
      ...
      >
        ...
        <DialogContent>
          <DialogContentText id="alert-dialog-description">
            Are you sure you want to delete this Car Type? <br />
      ...
      >
        ...
        <DialogContent>
          <DialogContentText id="alert-dialog-description">
            Are you sure you want to delete all the selected Car Types? <br />
