...
function Origins() {
  ...
  const [openPopupTitle, setOpenPopupTitle] = useState("New Origin Country"); // Customize
  ...
  const [pageHeader, setPageHeader] = useState("Origin Countries");
      ...
      renderCell: (params) => {
        return (
          ...
          >
            ...
            {userPermissions.includes("origin_countries_update") ? (
              <Button
                ...
                onClick={() => {
                  ...
                  setOpenPopup(true);
                  setOpenPopupTitle("Edit Origin Country");
                }}
              >
                Edit
              </Button>
            ...
            {userPermissions.includes("origin_countries_delete") ? (
              <Button
                className={classes.actionBtn}
                startIcon={<Delete />}
                color="secondary"
  ...
  const DeleteItem = () => {
    axios
      .delete(`/origin/countries/${itemToDelete}`)
      .then((res) => {
        setOpenDeleteDialog(false);
        setLoading(true);
        axios
          .get(
            `/origin/countries?page=${page}&per_page=${pageSize}&ordered_by=${sortModel[0].field}&sort_type=${sortModel[0].sort}`
          )
          .then((res) => {
  ...
  const MassDelete = () => {
    axios
      .post(`/origin/countries/mass/delete`, {
  ...
  useEffect(() => {
    ...
    if (!userIsSearching) {
      ...
      axios
        .post(
          `/origin/countries/search/name?page=${page}&per_page=${pageSize}&ordered_by=${sortModel[0].field}&sort_type=${sortModel[0].sort}`,
          {
  ...
  return (
    <React.Fragment>
        ...
        <Paper mb={2}>
          ...
          >
            <div style={{ display: "flex", alignItems: "flex-end" }}>
              {userPermissions.includes("origin_countries_add") ? (
                <Button
                  ...
                  onClick={() => {
                    setOpenPopup(true);
                    setSelectedItem("");
                    setOpenPopupTitle("New Origin Country");
                  }}
                  startIcon={<Add />}
                >
                  New Origin Country
        ...
        <Paper>
          <div style={{ width: "100%" }}>
            <DataGrid
              ...
              autoHeight={true}
              onRowClick={
                userPermissions.includes("origin_countries_show")
                  ? ({ row }) =>
                      history.push(`/product/origin-countries/${row.id}`)
                  : null
              }
      ...
      >
        ...
        <DialogContent>
          <DialogContentText id="alert-dialog-description">
            Are you sure you want to delete this Origin Country? <br />
      ...
      >
        ...
        <DialogContent>
          <DialogContentText id="alert-dialog-description">
            Are you sure you want to delete all the selected Origins? <br />
