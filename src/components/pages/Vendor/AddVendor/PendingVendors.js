...
function PendingVendors() {
  ...
  const [pageHeader, setPageHeader] = useState("Pending Vendors");
  ...
  const DeleteVendor = () => {
    axios
      ...
      .then((res) => {
        ...
        axios
          .get(
            `/count/pending/vendors?page=${page}&per_page=${pageSize}&ordered_by=${sortModel[0].field}&sort_type=${sortModel[0].sort}`
          )
          .then((res) => {
